...
class SSEData_MySQLp {
	...
	*/
	private function connect(){
		...
		$sourcename = 'mysql:host='.$host.';dbname='.$db.';';
		$this->conn = new PDO($sourcename, $user, $pass, array('charset'=>'utf8'));
	...
	*/
	private function check_reconnect(){
		if ($this->conn->getAttribute(PDO::ATTR_SERVER_INFO)=='MySQL server has gone away'){
			if(!$this->connect()){
...
//		}
	...
	*/
	private function prepare(){
		...
		$sql = "CREATE TABLE IF NOT EXISTS `sse_data_table` (`key` varchar(50) NOT NULL, `value` text, PRIMARY KEY (`key`) ) ENGINE=MEMORY DEFAULT CHARSET=utf8;";
//		echo $sql;
		$this->conn->exec($sql);
...
//		return (bool)(mysqli_query($this->conn,'CREATE TABLE IF NOT EXISTS `sse_data_table` (`key` varchar(50) NOT NULL, `value` text, PRIMARY KEY (`key`) ) ENGINE=MEMORY DEFAULT CHARSET=utf8;'));
	...
	*/
	public function get($key){
		...
		$sql = "SELECT * FROM `sse_data_table` WHERE `key` = :key;";
...
//		return $res['value'];
	...
	*/
	public function set($key,$value){
		if($this->get($key)){
			$sql = "UPDATE `sse_data_table` SET `value` = :value WHERE `key` = :key;";
			$stmt = $this->conn->prepare($sql);
			...
			$sql = "INSERT INTO `sse_data_table` SET `key` = :key, `value` = :value;";
			$stmt = $this->conn->prepare($sql);
			$stmt->bindParam(":value", $value );
			$stmt->bindParam(":key", $key );
			$stmt->execute();
...
//		}
	...
	*/
	public function delete($key){
		$sql = "DELETE FROM `sse_data_table` WHERE `key` == :key;";
		$stmt = $this->conn->prepare($sql);
		$stmt->bindParam(":key", $key );
		return $stmt->execute();
...
//register the module
SSEData::register('mysqlp','SSEData_MySQLp');
