...
//						var millis = data.incrementer();
					...
					data.update_function = function(sec) {
//						var millis = data.incrementer();
						var millis = sec * 1000;
						data.startTime = millis;
						data.elapsed = millis;
						data.target.data.elapsed = millis;
						data.target.data.startTime = millis;
						data.target.stopwatch('render');
					};
...
//            var $this = $(this),
//                data = $this.data('stopwatch');
            return $(this).data('stopwatch').elapsed;
        },
        
        getActive: function() {
            return $(this).data('stopwatch').active;
        },
		...
		setTime: function(seconds) {
            return this.each(function() {
				...
				var $this = $(this);
				data = $this.data('stopwatch');
				data.update_function(seconds);
				
//				data.elapsed = seconds*1000;
//				data.target.trigger('tick.stopwatch', [data.elapsed]);
				$this.data('stopwatch', data);
//				$this.stopwatch('start');
			});
        }
